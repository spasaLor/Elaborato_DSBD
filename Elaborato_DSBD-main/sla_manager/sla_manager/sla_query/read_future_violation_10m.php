...
if($num>0){
    ...
    $tempo = "1d";
    $campioni = 1440;
    $minuti_previsione = 10;
    $command = 'python query_future_violation.py node_'.$sla->Nome." ".$max_value." ".$min_value. " ".$tempo. " ".$campioni. " ".$minuti_previsione;
    $json = exec($command, $out, $status);
    $array = json_decode($json, true);
    $arr2 = array();

    $arr2["VIOLAZIONI PREVISTE"] = array();
    if(count($array)>0) {
        $item2 = array(
            "Numero di violazioni" => count($array)
            );
    array_push($arr2["VIOLAZIONI PREVISTE"], $item2);
    ...
    array_push($arr2["VIOLAZIONI PREVISTE"], $item3);
    }
